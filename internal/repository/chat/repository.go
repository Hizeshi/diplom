...
func (r *Repository) EnsureSession(ctx context.Context, sessionID, userID, platform string, authUserID *string) error {
	_, err := r.db.Exec(ctx, `
		INSERT INTO chat_sessions (session_id, user_id, platform, auth_user_id, is_human_mode)
		VALUES ($1, $2, $3, $4, false)
		ON CONFLICT (session_id) DO UPDATE
		SET updated_at = NOW()`,
		sessionID, userID, platform, authUserID)
	if err != nil {
		return fmt.Errorf("chatrepo: ensure session: %w", err)
	}
	return nil
}
...
// GetPublicHistory returns messages formatted for the public chat history endpoint.
func (r *Repository) GetPublicHistory(ctx context.Context, sessionID string) ([]chat.PublicMessage, error) {
	rows, err := r.db.Query(ctx, `
