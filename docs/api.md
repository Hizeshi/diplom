...
Вебхук от платёжной системы об успехе/отказе оплаты.
...
```json
{
  "success":    true,
  "orderId":    101,
  "paymentUrl": "https://payment.provider.kz/pay?order=101",
  "message":    "Заказ принят. Менеджер свяжется с вами."
}
```

> `paymentUrl` присутствует только при `paymentMethod: "card"` и если настроен платёжный шлюз. `message` присутствует при `"cash"`.
